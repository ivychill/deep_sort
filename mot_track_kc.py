...
from torchreid.feature_extractor_batch import Extractor
import os, cv2, time
from post_process import removeUnMoveLowConfObj,writeResult,removeSmallOrBigBbox


class KCTracker(object):
    def __init__(self, model_path='torchreid/checkpoint/reid_model.pth', gpu_ids='0', model_name='osnet_x1_0',
                 confidence_l=0.2, confidence_h=0.4, max_cosine_distance=0.2, max_iou_distance=0.7,
                 save_feature=False, use_filter=False,init_extractor=True, max_age=30, std_Q_w=1e-1, std_Q_wv=1e-3, std_R_w=5e-2):
    ...
    def getFeatureFromImage(self, bbox_tlwhcs, data, input_type):
        bbox_tlwhs = bbox_tlwhcs[:, 0:4]
        features = None
        if input_type == 'img':
            self.height, self.width = data.shape[:2]
            try:
                features = self._get_features_batch(bbox_tlwhs, data)
    ...
    def update(self,frame_id, bbox_tlwhcs, ori_img, input_type='img'):

        # 对输入数据进行过滤
        # 1.去除低confidence的
        # 2.通过处于跟踪状态的目标的预测位置和检测位置的iou过滤所有的候选框
        # print('ini boxs number:',len(bbox_tlwhs))
        ...
        features = self.getFeatureFromImage(bbox_tlwhcs_new, ori_img, input_type)
        ...
        detections = [Detection(bbox_tlwhs_new[i], conf, features[i]) for i, conf in enumerate(confidences_new)]
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append(np.array([track_id, x1, y1, x2, y2, conf]))
    ...
    def _get_features_batch(self, bbox_tlwhs, ori_img):
        imgs = []
        if self.width == None:
        ...
        for box in bbox_tlwhs:
            ...
            imgs.append(im)
        features = self.extractor(imgs, 20)
        return features
